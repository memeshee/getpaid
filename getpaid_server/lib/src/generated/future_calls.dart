...
import 'package:getpaid_server/src/generated/billing/chase_payload.dart'
    as _iwur04d6;
import 'package:serverpod/serverpod.dart' as _is;
import '../billing/chase_future_call.dart' as _iod2hmbx;
...
class FutureCalls extends _is.FutureCallDispatch<_FutureCallRef> {
  ...
  void initialize(
    ...
    var registeredFutureCalls = <String, _is.InvokableFutureCall>{
      'ChaseCheckChaseFutureCall': ChaseCheckChaseFutureCall(),
    };
...
class _FutureCallRef {
  ...
  late final chase = _ChaseFutureCallDispatcher(_invokeFutureCall);
}

class _ChaseFutureCallDispatcher {
  _ChaseFutureCallDispatcher(this._invokeFutureCall);

  final _InvokeFutureCall _invokeFutureCall;

  Future<void> checkChase(_iwur04d6.ChasePayload? payload) {
    return _invokeFutureCall(
      'ChaseCheckChaseFutureCall',
      payload,
    );
  }
}

class ChaseCheckChaseFutureCall extends _is.FutureCall<_iwur04d6.ChasePayload>
    implements _is.InvokableFutureCall<_iwur04d6.ChasePayload> {
  @override
  _ida.Future<void> invoke(
    _is.Session session,
    _iwur04d6.ChasePayload? payload,
  ) async {
    await _iod2hmbx.ChaseFutureCall().checkChase(
      session,
      payload,
    );
  }
